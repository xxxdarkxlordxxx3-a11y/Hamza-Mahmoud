...
interface InvestmentPlannerProps {
    onLaunchRiskQuiz: () => void;
    userContext: UserContext;
    details: any;
    setDetails: React.Dispatch<React.SetStateAction<any>>;
    plan: InvestmentPlan | null;
    setPlan: React.Dispatch<React.SetStateAction<InvestmentPlan | null>>;
}

const InvestmentPlanner: React.FC<InvestmentPlannerProps> = ({ 
    onLaunchRiskQuiz, 
    userContext,
    details,
    setDetails,
...
const FinancialToolsSection: React.FC<FinancialToolsSectionProps> = ({ onDone, userContext, activeSubTool, setActiveSubTool }) => {
    ...
    // Lifted state for Investment Planner
    const [investmentDetails, setInvestmentDetails] = useState({ goal: '', target: '', timeline: '', initial: '', monthly: '', risk: 'Medium' });
    ...
    const handleRiskAnalysisDone = (profile?: RiskProfile) => {
        if (profile) {
            ...
            // Update the persisted details immediately so it reflects in the planner
            setInvestmentDetails(prev => ({ ...prev, risk: mappedRisk }));
        }
                        ...
                        >
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    ...
                                    {activeSubTool === 'investment' && (
                                        <InvestmentPlanner 
                                            onLaunchRiskQuiz={() => setActiveSubTool('risk')} 
                                            userContext={userContext} 
                                            details={investmentDetails}
                                            setDetails={setInvestmentDetails}
                                            plan={investmentPlan}
                                            setPlan={setInvestmentPlan}
                                        />
                                    )}
